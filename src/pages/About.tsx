      ...
      <div className="container mx-auto px-4">
            ...
            <div className="space-y-6">
              ...
              {!prefersReducedMotion ? (
                <TextType
                  ...
                  typingSpeed={40}
                  pauseDuration={1800}
                  textColors={["var(--primary)", "var(--muted-foreground)"]}
                  cursorCharacter="_"
                />
              ) : (
              ...
              <div className="flex flex-wrap gap-4 pt-4">
                <a href="https://www.instagram.com/leonardossil/" target="_blank" rel="noopener noreferrer">
        ...
        <SectionReveal delay={0.3}>
          <div className="max-w-3xl mx-auto">
            ...
            <Stepper
              initialStep={1}
              disableStepIndicators={prefersReducedMotion}
              backButtonText="Prev"
              nextButtonText="Next"
              onFinalStepCompleted={() => undefined}
              className="bg-card/60 border border-border/60 rounded-3xl p-6"
              contentClassName="!px-0"
            >
              {timeline.map((item) => (
                <Step key={item.year}>
                  <div className="flex flex-col gap-3">
                    <span className="inline-flex w-fit px-3 py-1 rounded-full bg-primary/10 text-primary border border-primary/20">
                      {item.year}
                    </span>
                    <p className="text-fluid-lg font-medium text-foreground">{item.event}</p>
                  </div>
                </Step>
              ))}
            </Stepper>
