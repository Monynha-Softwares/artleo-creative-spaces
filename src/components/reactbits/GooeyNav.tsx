...
export const GooeyNav = ({
  ...
  const makeParticles = (element: HTMLElement) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const p = createParticle(i, t, d, particleR, particleCount, colors);
  ...
  const handleActivation = (index: number, liEl: HTMLLIElement) => {
  ...
  const handleClick: React.MouseEventHandler<HTMLButtonElement> = (event) => {
    const liEl = event.currentTarget.parentElement as HTMLLIElement | null;
    const index = Number(event.currentTarget.dataset.index ?? -1);
    if (!liEl || index === activeIndex || index < 0) return;
    event.preventDefault();
    handleActivation(index, liEl);
  };
  ...
  if (prefersReducedMotion) {
    return (
      <div className="gooey-nav-container reduced" ref={containerRef}>
        <nav>
          <ul>
